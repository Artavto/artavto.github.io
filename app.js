...
var S = {
  ...
  lotAuto: null,    // ставка, яку підставили МИ з «купити зараз»
};
...
function useCar(c) {
  ...
  /* Ціну лоту підставляємо ЛИШЕ якщо це «купити зараз» — фіксована сума
     без торгів. Для звичайного лота ціни не існує, поки хтось не
     поставив: це рішення людини, а не факт про авто. Підставити тут
     будь-яке інше число означало б вигадати ставку за клієнта.

     Але ставку від ПОПЕРЕДНЬОГО лота треба прибрати. Інакше знайшов
     одне авто з фіксованою ціною, потім шукаєш друге — і друге
     рахується за ціною першого. Саме на цьому я обпікся в минулому
     калькуляторі: «ціна лоту від попереднього авто».

     Те, що людина вписала САМА, лишаємо — це її рішення, і затирати
     його ми не маємо права. */
  var buyNow = 0;
  if (S.lotAuto && S.lot === S.lotAuto) {
    S.lot = null;
    $('#lot').value = '';
    S.lotAuto = null;
  }
  if (c.buyNow > 0 && !S.lot) {
    S.lot = c.buyNow;
    S.lotAuto = c.buyNow;      // позначаємо: це не людина вписала, а ми
    $('#lot').value = c.buyNow;
    buyNow = c.buyNow;
  }
  ...
  /* Підказка мусить описувати стан, а не момент пошуку. Раніше вона
     казала «лишилось вказати ставку» навіть коли ставка вже стояла — і
     це виглядало як недоробка. */
  if (miss.length) {
    lotMsg('Знайшли. Перевірте вручну: ' + miss.join(', ') + '.', 'er');
  } else if (buyNow) {
    lotMsg('Знайшли. Цей лот продається за фіксованою ціною ' +
      money(buyNow) + ' — вона вже стоїть у ставці. Змініть, якщо треба.', 'inf');
  } else if (S.lot) {
    lotMsg('Знайшли. Ставка ' + money(S.lot) + ' залишилась ваша — ' +
      'змініть, якщо для цього авто інша.', 'inf');
  } else {
    lotMsg('Знайшли. Лишилось вказати вашу ставку — скільки ви готові ' +
      'заплатити на торгах.', 'inf');
  }
}
...
/* Назви майданчиків у таблиці латиницею, а людина пише як думає —
   кирилицею. «Норфолк» проти «Norfolk» не збігається жодним символом,
   і пошук мовчав. Тому перекладаємо запит на латиницю й шукаємо обома. */
var LAT = {
  'а':'a','б':'b','в':'v','г':'h','ґ':'g','д':'d','е':'e','є':'ye','ж':'zh',
  'з':'z','и':'y','і':'i','ї':'yi','й':'y','к':'k','л':'l','м':'m','н':'n',
  'о':'o','п':'p','р':'r','с':'s','т':'t','у':'u','ф':'f','х':'h','ц':'ts',
  'ч':'ch','ш':'sh','щ':'sch','ю':'yu','я':'ya','ы':'y','э':'e','ё':'e',
  'ъ':'','ь':'',
  /* Апостроф буває трьох видів: прямий, друкарський і модифікатор.
     Telegram і клавіатура телефона ставлять не той, що на комп'ютері,
     тому враховуємо всі — інакше «Хʼюстон» не знаходився. */
  '\u0027':'', '\u2019':'', '\u02BC':'', '\u2018':'', '`':'',
};

/* Міста, яких транслітерація не дасть у принципі: в оригіналі вони
   пишуться зовсім інакше, ніж звучать. Список короткий і закриває ті,
   що справді є в таблиці. */
var CITY = {
  'фінікс':'phoenix', 'фенікс':'phoenix',
  'лос-анджелес':'los angeles', 'лос анджелес':'los angeles',
  'чикаго':'chicago', 'філадельфія':'philadelphia',
  'цинциннаті':'cincinnati', 'піттсбург':'pittsburgh', 'пітсбург':'pittsburgh',
  'детройт':'detroit', 'сіетл':'seattle', 'мілуокі':'milwaukee',
  'альбукерке':'albuquerque', 'луїсвілль':'louisville', 'луїсвіль':'louisville',
  'тусон':'tucson', 'мемфіс':'memphis', 'шарлотт':'charlotte', 'шарлот':'charlotte',
  'ролі':'raleigh', 'новий орлеан':'new orleans', 'ноксвілл':'knoxville',
  'баффало':'buffalo', 'буффало':'buffalo', 'балтимор':'baltimore',
  'бостон':'boston', 'хʼюстон':'houston', 'хьюстон':'houston',
  'нью-йорк':'ny -', 'нью-джерсі':'nj -',
};
...
function toLat(q) {
  ...
  for (var i = 0; i < q.length; i++) {
    var c = q[i];
    out += (LAT[c] !== undefined) ? LAT[c] : c;
  }
  return out;
}

/* Штати люди називають українською значно частіше, ніж міста. «Техас»
   має знаходити всі техаські майданчики, а не нічого. */
var STATE = {
  'алабама':'AL','аляска':'AK','аризона':'AZ','арканзас':'AR','каліфорнія':'CA',
  'колорадо':'CO','коннектикут':'CT','делавер':'DE','флорида':'FL','джорджія':'GA',
  'гавайї':'HI','айдахо':'ID','іллінойс':'IL','індіана':'IN','айова':'IA',
  'канзас':'KS','кентуккі':'KY','луїзіана':'LA','мен':'ME','меріленд':'MD',
  'массачусетс':'MA','мічиган':'MI','міннесота':'MN','міссісіпі':'MS','міссурі':'MO',
  'монтана':'MT','небраска':'NE','невада':'NV','нью-гемпшир':'NH','нью-джерсі':'NJ',
  'нью-мексико':'NM','нью-йорк':'NY','північна кароліна':'NC','північна дакота':'ND',
  'огайо':'OH','оклахома':'OK','орегон':'OR','пенсильванія':'PA','род-айленд':'RI',
  'південна кароліна':'SC','південна дакота':'SD','теннессі':'TN','техас':'TX',
  'юта':'UT','вермонт':'VT','вірджинія':'VA','вашингтон':'WA',
  'західна вірджинія':'WV','вісконсин':'WI','вайомінг':'WY',
};
...
function search(q) {
  ...
  var tries = [q];
  if (CITY[q]) tries.push(CITY[q]);
  var lat = toLat(q);
  if (lat !== q) tries.push(lat);
  if (STATE[q]) tries.push(STATE[q].toLowerCase() + ' -');

  var hit = list.filter(function (l) {
    var hay = (l.n + ' ' + (l.c || '') + ' ' + (l.z || '')).toLowerCase();
    for (var i = 0; i < tries.length; i++)
      if (hay.indexOf(tries[i]) > -1) return true;
    return false;
  });
  if (hit.length) return hit.slice(0, 40);

  /* Запасний прохід — тільки коли звичайний нічого не дав. Голосні при
     перекладі гуляють: «Гемптон» дає hempton, а в таблиці Hampton;
     «Хʼюстон» дає hyuston, а там Houston. Тому порівнюємо самі
     приголосні — вони при перекладі не міняються.

     Вмикається лише як остання спроба: точний запит має лишатись
     точним, інакше на «Dallas» посиплються Dulles і подібне. */
  var skel = bones(lat);
  if (skel.length < 3) return [];
  return list.filter(function (l) {
    return bones((l.n + ' ' + (l.c || '')).toLowerCase()).indexOf(skel) > -1;
  }).slice(0, 40);
}

/* Лишаємо приголосні. «y» теж прибираємо: у перекладі з кирилиці вона
   зʼявляється там, де в оригіналі її немає (юрій → yuriy). */
function bones(t) {
  return String(t).toLowerCase().replace(/[aeiouy]/g, '');
}
...
function drawFound(q) {
  ...
  /* Мовчання — найгірша відповідь. Людина не розуміє, чи вона написала
     не так, чи такого майданчика немає, чи все зламалось. Кажемо прямо. */
  if (!res.length) {
    var typed = String(q || '').trim();
    if (typed.length < 2) { box.innerHTML = ''; box.className = 'found'; return; }
    box.className = 'found empty';
    box.innerHTML = '<div>Нічого не знайшлось.' +
      (/[а-яїієґ]/i.test(typed)
        ? ' Спробуйте англійською — у таблиці назви латиницею.'
        : ' Перевірте написання або спробуйте назву штату.') +
      '</div>';
    return;
  }
  box.className = 'found';
...
function start() {
  ...
  [['lot','lot'],['fee','fee'],['year','year'],['cc','cc'],['kwh','kwh']].forEach(function (p) {
    $('#' + p[0]).addEventListener('input', function () {
      S[p[1]] = num(this.value);
      if (p[1] === 'lot') {
        /* Людина вписала ставку сама — далі це її число, а не наше. */
        S.lotAuto = null;
        /* І прибираємо підказку «лишилось вказати ставку»: вона стояла з
           моменту пошуку й після появи суми виглядала як недоробка. */
        var m = $('#lotMsg');
        if (m && m.textContent.indexOf('вашу ставку') > -1) lotMsg('');
      }
      drawSum();
    });
  });
